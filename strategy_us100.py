"""Strategie US100 H1 — H1 trend filter (EMA30 via M30 proxy) + trailing stop at 2x SL.

Fonctionnement :
1. Utilise le modele JEPA M15 (checkpoints US100_m15 existants)
2. Filtre H1 (proxy M30) : ne trader que dans la direction de la tendance (EMA30 sur M30 ~ EMA30 H1)
...
5. Comment trades : "EVA-US100" (via TRADE_COMMENT env var)
"""
...
# US100 specifics
SL_DIST_US100 = 0.80       # 80 pips SL distance
SPREAD_US100 = 0.03        # ~3 pips spread for US100
...
def analyser_tendance_h1(bars: list[dict]) -> tuple[int, float]:
    """Analyse la tendance via EMA30 sur M30 (equivalent EMA30 H1).
    ...
    ema = calculer_ema(closes, 40)  # EMA30 on M30 ~ EMA30 on H1
    if not ema:
        return 0, 0.0
    last_ema = ema[-1]
    last_close = closes[-1]
...
class OrchestrateurUS100_H1(OrchestrateurEVA):
    """Orchestrateur US100 avec filtre H1 (EMA30) + trailing stop 1.5x SL."""
    ...
    def _trailing_stop(self, prix_actuel: float, atr: float) -> None:
        ...
        """
        entry_buffer = SL_DIST_US100 * 2  # 1.60 — seuil d'activation du trailing
            ...
            for pos in pos_sym:
                ...
                if ptype == "BUY":
                    if profit >= entry_buffer:
                        breakeven_sl = round(pos_price + SPREAD_US100, 3)
    ...
    def tick(self) -> None:
        ...
        # 1. Pipeline JEPA (M15 — meme checkpoint US100_m15)
        ...
        if direction != 0 and ordre_brut.lot >= self.sanitizer.limites.lot_min:
            ...
            if direction_int > 0:
                sl = round(base_price - SL_DIST_US100, 3)
            else:
                sl = round(base_price + SL_DIST_US100, 3)
...
def main() -> None:
    ...
    journal.info("Demarrage US100-H1: %s (max-pos=%d)", SYMBOLE, args.max_pos)
    OrchestrateurUS100_H1(
