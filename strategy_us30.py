"""Strategie US30 H1 — H1 trend filter (EMA20 via M30 proxy) + trailing stop at 2x SL.

Fonctionnement :
1. Utilise le modele JEPA M15 (checkpoints US30_m15 existants)
2. Filtre H1 (proxy M30) : ne trader que dans la direction de la tendance (EMA20 sur M30 ~ EMA20 H1)
...
5. Comment trades : "EVA-US30" (via TRADE_COMMENT env var)
"""
...
# US30 specifics
SL_DIST_US30 = 0.80       # 80 pips SL distance
SPREAD_US30 = 0.03        # ~3 pips spread for US30
...
def analyser_tendance_h1(bars: list[dict]) -> tuple[int, float]:
    """Analyse la tendance via EMA20 sur M30 (equivalent EMA20 H1).
    ...
    ema = calculer_ema(closes, 40)  # EMA20 on M30 ~ EMA20 on H1
    if not ema:
        return 0, 0.0
    last_ema = ema[-1]
    last_close = closes[-1]
...
class OrchestrateurUS30_H1(OrchestrateurEVA):
    """Orchestrateur US30 avec filtre H1 (EMA20) + trailing stop 2.0x SL."""
    ...
    def _trailing_stop(self, prix_actuel: float, atr: float) -> None:
        ...
        """
        entry_buffer = SL_DIST_US30 * 2  # 1.60 — seuil d'activation du trailing
            ...
            for pos in pos_sym:
                ...
                if ptype == "BUY":
                    if profit >= entry_buffer:
                        breakeven_sl = round(pos_price + SPREAD_US30, 3)
    ...
    def tick(self) -> None:
        ...
        # 1. Pipeline JEPA (M15 — meme checkpoint US30_m15)
        ...
        if direction != 0 and ordre_brut.lot >= self.sanitizer.limites.lot_min:
            ...
            if direction_int > 0:
                sl = round(base_price - SL_DIST_US30, 3)
            else:
                sl = round(base_price + SL_DIST_US30, 3)
...
def main() -> None:
    ...
    journal.info("Demarrage US30-H1: %s (max-pos=%d)", SYMBOLE, args.max_pos)
    OrchestrateurUS30_H1(
