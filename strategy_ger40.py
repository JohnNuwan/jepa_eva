"""Strategie GER40 H1 — H1 trend filter (EMA25 via M30 proxy) + trailing stop at 2x SL.

Fonctionnement :
1. Utilise le modele JEPA M15 (checkpoints GER40_m15 existants)
2. Filtre H1 (proxy M30) : ne trader que dans la direction de la tendance (EMA25 sur M30 ~ EMA25 H1)
...
5. Comment trades : "EVA-GER40" (via TRADE_COMMENT env var)
"""
...
# GER40 specifics
SL_DIST_GER40 = 0.80       # 80 pips SL distance
SPREAD_GER40 = 0.03        # ~3 pips spread for GER40
...
def analyser_tendance_h1(bars: list[dict]) -> tuple[int, float]:
    """Analyse la tendance via EMA25 sur M30 (equivalent EMA25 H1).
    ...
    ema = calculer_ema(closes, 40)  # EMA25 on M30 ~ EMA25 on H1
    if not ema:
        return 0, 0.0
    last_ema = ema[-1]
    last_close = closes[-1]
...
class OrchestrateurGER40_H1(OrchestrateurEVA):
    """Orchestrateur GER40 avec filtre H1 (EMA25) + trailing stop 2.5x SL."""
    ...
    def _trailing_stop(self, prix_actuel: float, atr: float) -> None:
        ...
        """
        entry_buffer = SL_DIST_GER40 * 2  # 1.60 — seuil d'activation du trailing
            ...
            for pos in pos_sym:
                ...
                if ptype == "BUY":
                    if profit >= entry_buffer:
                        breakeven_sl = round(pos_price + SPREAD_GER40, 3)
    ...
    def tick(self) -> None:
        ...
        # 1. Pipeline JEPA (M15 — meme checkpoint GER40_m15)
        ...
        if direction != 0 and ordre_brut.lot >= self.sanitizer.limites.lot_min:
            ...
            if direction_int > 0:
                sl = round(base_price - SL_DIST_GER40, 3)
            else:
                sl = round(base_price + SL_DIST_GER40, 3)
...
def main() -> None:
    ...
    parser.add_argument("--symbol", default="GER40")
    ...
    journal.info("Demarrage GER40-H1: %s (max-pos=%d)", SYMBOLE, args.max_pos)
    OrchestrateurGER40_H1(
